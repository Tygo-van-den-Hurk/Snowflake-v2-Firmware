          ...
          derivation =
            ...
            pkgs.stdenv.mkDerivation rec {
              ...
              buildPhase = ''
                ...
                file="${src}/features/version_print/feature.h" 
                FIRMWARE_VERSION=$(awk -F'"' '
                  /#define[[:space:]]+SNOWFLAKE_FIRMWARE_VERSION/ { print $2; found=1 }
                  END { if (!found) exit 1 }
                ' "$file") || {
                  echo "SNOWFLAKE_FIRMWARE_VERSION not found in $file"
                  exit 1
                }
